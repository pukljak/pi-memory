...
export async function snapshotCodebase(store: MemoryStore, root: string, domainId: string, deep = true) {
  ...
  const items: any[] = [
    ...
    { id: uid(), kind: "fact", scope: "domain", domainId, text: `Runbook (${domainId}): use /memory.status, /memory.ui, /memory.explore --deep and /memory.snapshot to refresh mapped knowledge; README=${readme || "not found"}`, confidence: 0.84, source: "memory_snapshot", createdAt: now, updatedAt: now },
